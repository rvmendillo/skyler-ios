...
@MainActor final class BattleSession:NSObject,ObservableObject {
    ...
    init(hero:Int,difficulty:Difficulty,spell:BattleSpell,practice:Bool,renderer:ArenaRenderer){self.renderer=renderer;game=ArenaSimulation(hero:hero,difficulty:difficulty,spell:spell,practice:practice);super.init();renderer.cameraPoint=game.player.p
        if ProcessInfo.processInfo.arguments.contains("--smoke-battle") {
            ...
            game.player.p=V2(46,46);renderer.cameraPoint=game.player.p;game.attacking=true
        }
    }
    ...
    @objc func frame(_ link:CADisplayLink){let dt=lastTime==0 ? 1.0/60:min(0.05,link.timestamp-lastTime);lastTime=link.timestamp;game.step(dt);renderer.update(game,dt:dt)
        let cues=game.sounds;game.sounds=[];for sound in cues {ArenaAudio.shared.play(sound);if haptics {if sound=="skill" || sound=="ultimate"{impact.impactOccurred(intensity:sound=="ultimate" ? 1:0.6)}else if sound=="level" || sound=="purchase"{notification.notificationOccurred(.success)}}};hudTime+=dt;if hudTime>0.10{revision+=1;hudTime=0}
    ...
    func pause(_ value:Bool){game.paused=value;game.movement = .zero;game.attacking=false;game.aimingSkill=nil;game.aim=nil;lastTime=0;if value{ArenaAudio.shared.stop()}else{ArenaAudio.shared.start()};revision+=1}
}
...
struct HeroPreview:UIViewRepresentable {
    let hero:Int
    func makeUIView(context:Context)->SCNView{let v=SCNView();v.backgroundColor = .clear;v.isOpaque=false;v.antialiasingMode = .multisampling4X;v.allowsCameraControl=true;v.autoenablesDefaultLighting=false;v.isPlaying=true;v.preferredFramesPerSecond=30;return v}
                ...
                ZStack {
                    ...
                    HStack(spacing:0){
                        VStack(alignment:.leading,spacing:14){
                            ...
                            Text("CHOOSE YOUR CHAMPION  /  12").font(.system(size:9,weight:.bold)).tracking(1.3).foregroundColor(.white.opacity(0.55))
                            ...
                            HStack{Text("OFFLINE 5v5 • REFORGED 2.1").font(.system(size:8,weight:.bold)).foregroundColor(cyan);Spacer();Button{help=true}label:{Image(systemName:"questionmark.circle")}.accessibilityIdentifier("guide")}
                        }.frame(width:312).padding(.leading,50).padding(.vertical,18)
                        ZStack {RadialGradient(colors:[Color(UIColor(rgb:Roster.heroes[hero].color)).opacity(0.16),.clear],center:.center,startRadius:10,endRadius:180);Circle().stroke(gold.opacity(0.16),lineWidth:1).frame(width:215,height:215).offset(y:-32);HeroPreview(hero:hero)}.frame(width:250,height:390)
...
struct ArenaMatch:View {
        ...
        ZStack {
            ...
            GeometryReader{geo in let s=min(geo.size.width/932,geo.size.height/430)
                ZStack {
                    ...
                    if g.time<5{Text("Protect your lanes. Destroy the enemy Nexus.").font(.system(size:12,weight:.semibold)).padding(10).panel().position(x:466,y:73)}
                    ...
                    Text(objectiveText).font(.system(size:7,weight:.medium,design:.monospaced)).foregroundColor(.white.opacity(0.75)).padding(5).panel().position(x:130,y:214)
                    targetSelector.position(x:818,y:181)
                    if g.player.alive,g.units.contains(where:{$0.structure && $0.team==1 && $0.alive && $0.target==g.playerID && $0.p.distance(g.player.p)<$0.range}) {Label("TURRET TARGETING YOU",systemImage:"exclamationmark.triangle.fill").font(.system(size:10,weight:.black)).foregroundColor(.pink).padding(7).panel().position(x:466,y:112)}
                    ...
                    roundAction("Regen","cross.fill",g.player.regenCD,color:.green){g.regenPlayer()}.position(x:304,y:382)
                    ...
                    if g.aimingSkill != nil{Text("DRAG TO AIM  ·  RELEASE TO CAST  ·  DRAG FAR TO CANCEL").font(.system(size:8,weight:.bold)).padding(8).panel().position(x:467,y:296)}
    ...
    var targetSelector:some View {
        HStack(spacing:5){ForEach(g.heroes.filter{$0.team==1 && $0.alive && g.visible($0,to:0) && $0.p.distance(g.player.p)<=20}){enemy in
            Button{g.lockTarget(enemy.id);session.revision+=1}label:{VStack(spacing:2){ZStack{Circle().fill(ink.opacity(0.9)).overlay(Circle().stroke(g.selectedTarget==enemy.id ? gold:Color(UIColor(rgb:enemy.def.color)).opacity(0.6),lineWidth:g.selectedTarget==enemy.id ? 2:1));Text(String(enemy.name.prefix(1))).font(.system(size:14,weight:.black,design:.serif)).foregroundColor(Color(UIColor(rgb:enemy.def.color)))}.frame(width:30,height:30);meter(enemy.hp/enemy.maxHP,.pink,height:2).frame(width:27)}}.buttonStyle(.plain).accessibilityLabel("Lock \(enemy.name)").accessibilityIdentifier("target-\(enemy.hero)")
    ...
    var settingsPanel:some View {
        NavigationStack {Form {
            Section("Controls") {
                Picker("Attack priority",selection:Binding(get:{g.targetPriority},set:{g.targetPriority=$0;session.revision+=1})){ForEach(TargetPriority.allCases,id:\.self){Text($0.rawValue).tag($0)}}
                Toggle("Approach targets while holding Attack",isOn:Binding(get:{g.attackAssist},set:{g.attackAssist=$0;session.revision+=1}))
                Toggle("Skill and purchase haptics",isOn:Binding(get:{session.haptics},set:{session.haptics=$0;session.revision+=1}))
                Text("Tap an enemy's portrait to lock on. Tap again to clear. Minion and turret buttons always respect their target type. Moving the joystick overrides automatic approach.").font(.caption)
            }
            Section("Presentation") {
                Toggle("Sound and music",isOn:$audio).onChange(of:audio){v in ArenaAudio.shared.enabled=v;if !v{ArenaAudio.shared.stop()}}
                Toggle("Shadows and bloom",isOn:$quality).onChange(of:quality){session.renderer.setQuality($0)}
                Text("The match pauses while settings are open.");Button("Controls and rules"){controlHelp=true}
            }
            if g.practice {Section("Practice") {
                Button("Restore health, mana and cooldowns"){g.player.hp=g.player.maxHP;g.player.mana=g.player.maxMana;g.player.cooldowns=[0,0,0];g.player.spellCD=0}
                Button("Add 5,000 gold"){g.player.gold+=5000}
            }}
            Section{Button("Leave match",role:.destructive){settings=false;exit()}}
        }.navigationTitle("Settings").toolbar{ToolbarItem(placement:.confirmationAction){Button("Resume"){settings=false}}}.sheet(isPresented:$controlHelp){GuideView()}}
    }
...
struct AbilityControl:View {
    ...
    var body:some View {
        VStack(spacing:3){ZStack {
            ...
            if u.cooldowns[index]>0 {Circle().trim(from:0,to:max(0,min(1,u.cooldowns[index]/(a.cooldown*(1-u.cdr))))).stroke(.white.opacity(0.65),style:StrokeStyle(lineWidth:3,lineCap:.round)).rotationEffect(.degrees(-90)).padding(4)}
